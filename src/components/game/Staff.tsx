...
const Staff: React.FC<StaffProps> = ({
  ...
  const clefX = 40;
  const keySignatureX = clefX + 60;
  const noteX = keySignatureX + (keySignature.sharps.length + keySignature.flats.length) * 15 + 40;

  // 음표 위치 계산
  const notePosition = note ? getNotePositionOnStaff(note, clef) : null;
  const noteY = notePosition !== null ? staffStartY + (8 - notePosition) * (staffLineSpacing / 2) : 0;

  // 보조선 계산
  const ledgerLines = notePosition !== null && needsLedgerLines(notePosition) 
  ...
  return (
    <div className={`staff-container ${className}`}>
      <svg width={width} height={height} className="staff-svg">
        ...
        {/* 음자리표 */}
        <g transform={`translate(${clefX}, ${staffStartY + staffLineSpacing * 2})`}>
        ...
        {keySignature.sharps.length > 0 && (
          <g>
            {keySignature.sharps.map((sharpNote, index) => (
              <g key={`sharp-${index}`} transform={`translate(${keySignatureX + index * 15}, ${getAccidentalY(sharpNote, clef, staffStartY, staffLineSpacing, 'sharp')})`}>
                <SharpSymbol />
              </g>
            ))}
          </g>
        ...
        {keySignature.flats.length > 0 && (
          <g>
            {keySignature.flats.map((flatNote, index) => (
              <g key={`flat-${index}`} transform={`translate(${keySignatureX + index * 15}, ${getAccidentalY(flatNote, clef, staffStartY, staffLineSpacing, 'flat')})`}>
                <FlatSymbol />
              </g>
            ))}
          </g>
...
const TrebleClef: React.FC = () => (
  <g>
    <path
      d="M-10,-30 Q-5,-35 0,-30 Q5,-25 5,-15 Q5,-5 0,0 Q-5,5 -10,0 Q-15,-5 -15,-15 Q-15,-25 -10,-30 Z M0,-15 Q10,-10 15,0 Q20,10 15,20 Q10,30 0,25 Q-10,20 -15,10 Q-20,0 -15,-10 Q-10,-20 0,-15 Z"
      fill="#000"
      transform="scale(0.8)"
    />
  </g>
...
const BassClef: React.FC = () => (
  <g>
    ...
    <circle cx="-15" cy="10" r="3" fill="#000" />
    <path
      d="M-25,-20 Q-20,-25 -10,-20 Q0,-15 5,-5 Q10,5 5,15 Q0,25 -10,20 Q-20,15 -25,5 Q-30,-5 -25,-15 Q-20,-25 -25,-20 Z"
      fill="#000"
      transform="scale(0.6)"
    />
  </g>
...
// 알토음자리표 SVG
const AltoClef: React.FC = () => (
  <g>
    <rect x="-15" y="-30" width="4" height="60" fill="#000" />
    <rect x="-8" y="-20" width="4" height="40" fill="#000" />
    <rect x="5" y="-30" width="4" height="60" fill="#000" />
    <rect x="12" y="-20" width="4" height="40" fill="#000" />
  </g>
);

// 테너음자리표 SVG
const TenorClef: React.FC = () => (
  <g>
    <rect x="-15" y="-20" width="4" height="40" fill="#000" />
    <rect x="-8" y="-30" width="4" height="60" fill="#000" />
    <rect x="5" y="-20" width="4" height="40" fill="#000" />
    <rect x="12" y="-30" width="4" height="60" fill="#000" />
  </g>
);
...
const QuarterNote: React.FC = () => (
  <g>
    <ellipse cx="0" cy="0" rx="8" ry="6" fill="#000" transform="rotate(-20)" />
    <rect x="7" y="-30" width="2" height="30" fill="#000" />
  </g>
);
...
const FlatSymbol: React.FC = () => (
  <g>
    ...
    <path
      d="M1,-5 Q8,-8 12,-2 Q16,4 12,10 Q8,16 1,13 Z"
      fill="#000"
    />
  </g>
);
