...
import { Note, NoteName, SolfegeNote, Octave } from '@/types/music';
import { playPianoNote } from '@/lib/music/audio';
import { NOTE_NAME_MAPPING, SOLFEGE_TO_NOTE_MAPPING, SOLFEGE_NOTES } from '@/lib/music/constants';
...
const SolfegeKeyboard: React.FC<SolfegeKeyboardProps> = ({
  ...
  // 키 클릭 핸들러
  const handleKeyClick = useCallback(async (solfege: SolfegeNote, octave: Octave) => {
    if (disabled) return;
  ...
  // 선택된 키인지 확인
  const isKeySelected = useCallback((solfege: SolfegeNote, octave: Octave): boolean => {
    if (!selectedNote) return false;
    
    const expectedSolfege = NOTE_NAME_MAPPING[selectedNote.name];
    return expectedSolfege === solfege && 
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {SOLFEGE_TO_NOTE_MAPPING[solfege]}{octave}
                    </div>
                  </div>
                </button>
              );
          ...
          <div className="mt-6 text-center text-sm text-gray-600">
            ...
            {selectedNote && (
              <p className="mt-2 font-medium text-blue-600">
                선택된 음표: {NOTE_NAME_MAPPING[selectedNote.name]} ({selectedNote.name}
