...
const PianoKeyboard: React.FC<PianoKeyboardProps> = ({
  ...
  const generateKeys = useCallback((): PianoKey[] => {
    ...
    return keys;
  }, [startOctave, endOctave, pressedKeys]);
  ...
  // 선택된 키인지 확인
  const isKeySelected = useCallback((key: PianoKey): boolean => {
    if (!selectedNote) return false;
    
    const keyNote = key.note.includes('#') ? key.note.replace('#', '') : key.note;
    const keyAccidental = key.note.includes('#') ? 'sharp' : 'natural';
    
    return selectedNote.name === keyNote &&
           selectedNote.accidental === keyAccidental &&
           selectedNote.octave === key.octave;
  }, [selectedNote]);
